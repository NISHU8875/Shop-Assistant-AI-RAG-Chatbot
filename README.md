...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│              E-COMMERCE CHATBOT SYSTEM (ShopAssist)                     │
│                   (Multi-Intent RAG + SQL Agent)                        │
└─────────────────────────────────────────────────────────────────────────┘
                    ...
                    ▼             ▼             ▼
        ┌───────────────┐ ┌─────────────┐ ┌──────────────┐
        │  FAQ INTENT   │ │ SQL INTENT  │ │ CHITCHAT     │
        │  route.name   │ │ route.name  │ │ INTENT       │
        │  = 'faq'      │ │ = 'sql'     │ │ route.name   │
        │               │ │             │ │ = 'chitchat' │
        └───────┬───────┘ └──────┬──────┘ └──────┬───────┘
                │                │                │
                ▼                ▼                ▼
    ┌───────────────────┐ ┌────────────────┐ ┌──────────────────┐
    │  faq_chain()      │ │ sql_chain()    │ │ chitchat_chain() │
    │  (faq.py)         │ │ (sql.py)       │ │ (chitchat.py)    │
    └─────────┬─────────┘ └────────┬───────┘ └────────┬─────────┘
              │                    │                   │
              │                    │                   │
    ┌─────────▼──────────┐ ┌──────▼─────────┐ ┌─────▼──────────┐
    │   RAG PIPELINE     │ │ TEXT-TO-SQL    │ │CONVERSATIONAL  │
    │                    │ │ PIPELINE       │ │AI              │
    │  1. get_relevant_  │ │                │ │                │
    │     qa(query)      │ │ 1. generate_   │ │• get_current_  │
    │                    │ │    sql_query() │ │  datetime_     │
    │  2. ChromaDB Query │ │    (Groq LLM#1)│ │  info()        │
    │     (Top-3)        │ │                │ │                │
    │                    │ │ 2. Regex       │ │• System Prompt │
    │  3. generate_      │ │    Extract     │ │  + DateTime    │
    │     answer()       │ │    <SQL></SQL> │ │  Context       │
    │     (Groq LLM)     │ │                │ │                │
    │                    │ │ 3. run_query() │ │• Groq LLM      │
    │                    │ │    (SQLite)    │ │  Temperature:  │
    │                    │ │                │ │  0.7           │
    │                    │ │ 4. to_dict()   │ │                │
    │                    │ │                │ │                │
...
│ DATA STORES                                                              │
│ ──────────────────────────────────────────────────────────────────────  │
│ • ChromaDB: FAQ embeddings (collection: 'faqs')                         │
│ • SQLite: Product data (db.sqlite, table: product)                      │
│ • Session State: Chat history (st.session_state.messages)               │
└─────────────────────────────────────────────────────────────────────────┘
